...
onMounted(async () => {
  AnList.value = await stage.UserfetchId(announcementById.value);
})
</script>
...
<template>
  ...
  <div class="frame" v-if="stage.mode === 'active'">
    <div class="ann-item">
      ...
      <h3 class="ann-description">Description : <span id="dis"> {{ AnList.announcementDescription }}</span></h3> <br>
    </div>
    <router-link :to="{ name: 'UserList' }"><button id="btback" class="ann-button">Back</button></router-link>
  </div>
  ...
  <div class="frame" v-if="stage.mode === 'close'">
    <div class="ann-item">
      ...
      <h3>Close On: <span class="ann-close-date"> {{ changeTime(AnList.closeDate) ? changeTime(AnList.closeDate) : '-'
      }}</span></h3>
    </div>
